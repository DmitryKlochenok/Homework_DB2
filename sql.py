...
app = Flask(__name__)
app.config["SQLALCHEMY_DATABASE_URI"] = 'sqlite:///homework2.db'
...
class Users(db.Model):
    ...
    executor = relationship("Users", foreign_keys=["executor_id"])
...
def orders_id_page(id):
    query = Orders.query.get(id)
    result = []
    for item in query:
        result.append({
            "id": item.id,
...
def add_user_page():
    if request.method == "POST":
        ...
        user_from_page = Users(id=id, first_name=first_name, last_name=last_name, age=age, email=email, role=role, phone=phone)
...
def update_user_page(id):
    ...
    new_user_data = json.load(request.get_json())
